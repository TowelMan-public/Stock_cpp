...
LRESULT CALLBACK UW::UserListProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
	HDC hdc;
	PAINTSTRUCT ps;
	HWND hh;
...
void UW::Create(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
	if (DB::GetIsLogin()) {
		//内容の取得(DB部から)
		acts = DB::GetUser();
...
void UW::Paint(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp, HDC hdc){
	if (DB::GetIsLogin()) {
		for (int i = 0; i < 5; i++) {
...
void UW::Vscroll(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
	switch (LOWORD(wp)) {
	case SB_TOP:
		scr.nPos = scr.nMin;
		break;
...
void UW::Comand(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp){
	int idx;
	int id;
	if (LOWORD(wp) >= 1 && LOWORD(wp) <= 5) {
		idx = (nowTopY * -1) + (LOWORD(wp) - 1);
		id = DB::GetTalkByUser(acts[idx].number);
		MW::UpdateChat(id);
	}
}